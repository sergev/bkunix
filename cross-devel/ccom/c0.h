...
union	tree *tree(int);
char	*copnum(int);
union	tree *convert(union tree *, int, int, int);
union	tree *chkfun(union tree *);
union	tree *disarray(union tree *);
union	tree *block(int, int, int *, union str *, union tree *, union tree *);
union	tree *cblock(int);
union	tree *fblock(int, char *);
char	*Dblock(int);
char	*Tblock(int);
char	*starttree(void);
union	tree *pexpr(int);
union	str *strdec(int, int);
union	tree *xprtype(void);
struct	nmlist *pushdecl(struct nmlist *);
unsigned hash(char *);
union	tree *structident(union tree *, union tree *);
struct	nmlist *gentemp(int);
union	tree *nblock(struct nmlist *);

/* Used in c00.c */
void error(const char *, ...);
void blkend(void);
void build(int);
int decl1(int, struct nmlist *, int, struct nmlist *);
void endtree(char *);
void errflush(int);
void extdef(void);
int findkw(void);
int getcc(void);
int getkeywords(int *, struct nmlist *);
int getnum(void);
int mapch(int);
int nextchar(void);
void outcode(char *, ...);
...
/* Used in c01.c */
int length(union tree *);
int fold(int, union tree *, union tree *);
int decref(int);
int incref(int);
void setype(union tree *, int, union tree *);
void chkw(union tree *, int);
void chklval(union tree *);
int lintyp(int);
void assignop(int, union tree *, union tree *);
void werror(char *, ...);
int plength(union tree *);
int rlength(union tree *);

/* Used in c02.c */
int symbol(void);
void setinit(struct nmlist *);
void cfunc(void);
int cinit(struct nmlist *, int, int);
void statement(void);
int declist(int);
void funchead(void);
void branch(int);
void label(int);
void putstr(int, int);
void strinit(struct nmlist *, int);
void rcexpr(union tree *);
void blockhead(void);
int simplegoto(void);
void dogoto(void);
void doret(void);
void cbranch(union tree *, int, int);
void chconbrk(int);
int conexp(void);
void pswitch(void);
int forstmt(void);
void redec(void);
int goodreg(struct nmlist *);
void prste(struct nmlist *);
void nameconflict(struct nmlist *, struct nmlist *);

/* Used in c03.c */
int declare(int, struct nmlist *, int);
int align(int, int, int);
void decsyn(int);
int getype(struct tdim *, struct nmlist *);
void typov(void);

void treeout(union tree *, int);
